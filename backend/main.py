...
async def run_workflow(params: UserInput):
    ...
    try:
        ...
        # The final workflow result is now a dictionary containing all fields
        final_result = await client.get_workflow_result(workflow_id=workflow_id, run_id=run_id)
        ...
        raise HTTPException(
            status_code=500,
            detail="Failed to connect to Restack engine or run workflow."
        )
